...
import { fetchJobs, fetchJobsApplied, fetchJobsHired, fetchSingleJob } from "../reducers/job.slice";
...
export const getSingleJobApplied = (id) => async (dispatch) => {
    var job = db.collection("Jobs").doc('QibZGH8ka9krCLHuc4VG');

    job.get().then((doc) => {
    if (doc.exists) {
        console.log("Document data:", doc.data());
        dispatch(fetchJobsApplied(doc.data()));
    } else {
        console.log("No such document!");
    }
}).catch((error) => {
...
export const getSingleJobHired = (id) => async (dispatch) => {
    var job = db.collection("Jobs").doc('QibZGH8ka9krCLHuc4VG');

    job.get().then((doc) => {
    if (doc.exists) {
        console.log("Document data:", doc.data());
        dispatch(fetchJobsHired(doc.data()));
    } else {
        console.log("No such document!");
    }
}).catch((error) => {
